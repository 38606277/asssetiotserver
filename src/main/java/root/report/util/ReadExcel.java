...
//        List<Map<String, Object>> userList = new LinkedList<Map<String, Object>>();
    ...
    private List<Map<String, Object>> readExcelValue(Workbook wb) {
        ...
        if (rowHeader!= null) {
            for (int h = 0; h < this.totalCells; h++) {
                Cell cell = rowHeader.getCell(h);
                headerList.add(h,cell.getStringCellValue());
            }
        }
        // 循环Excel的列
                ...
                if (null != cell) {
                   if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
                       if(HSSFDateUtil.isCellDateFormatted(cell)) {
                           ...
                           map.put(headerList.get(c),formater.format(d));

                       } else {
                           String name = String.valueOf(cell.getNumericCellValue());
                           BigDecimal lastName=new BigDecimal("0"+name.substring(name.length() - 2 > 0 ? name.length() - 2 : name.length()));
                           int i=lastName.compareTo(BigDecimal.ZERO);
                           if(i==0) {
                               map.put(headerList.get(c), name.substring(0, name.length() - 2 > 0 ? name.length() - 2 : 1));
                           }else{
                               map.put(headerList.get(c), name);
                           }
                       }
                    } else {
                        map.put(headerList.get(c), cell.getStringCellValue());// header行名称
                    }
//                    if (c == 0) {
//                        // 如果是纯数字,比如你写的是25,cell.getNumericCellValue()获得是25.0,通过截取字符串去掉.0获得25
//                        if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
//                            String name = String.valueOf(cell.getNumericCellValue());
//                            map.put(headerList.get(c), name.substring(0, name.length() - 2 > 0 ? name.length() - 2 : 1));// 名称
//                        } else {
//                            map.put(headerList.get(c), cell.getStringCellValue());// 名称
//                        }
//                    } else if (c == 1) {
//                        if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
